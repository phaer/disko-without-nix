...
use anyhow::anyhow;
use crate::{disk::Disk, mdadm::Mdadm, zfs::Zpool, lvm::LvmVg, nodev::Nodev};
...
impl TryFrom<&str> for DevicePath {
    ...
    fn try_from(value: &str) -> Result<Self, Self::Error> {
        ...
        let capture = captures.get(0).ok_or_else(|| anyhow!("device_path regex does not match"))?;
...
fn test_device_path_try_from() {
    assert_eq!(DevicePath("/dev/sdx".to_string(), None), "/dev/sdx".try_into().unwrap());
    assert_eq!(DevicePath("/dev/sdx".to_string(), Some(1)), "/dev/sdx1".try_into().unwrap());
    assert_eq!(DevicePath("/dev/disk/by-id/scsi-0QEMU_QEMU_HARDDISK_19353493".to_string(), None),
               "/dev/disk/by-id/scsi-0QEMU_QEMU_HARDDISK_19353493".try_into().unwrap());
    assert_eq!(DevicePath("/dev/disk/by-id/scsi-0QEMU_QEMU_HARDDISK_19353493".to_string(), Some(3)),
               "/dev/disk/by-id/scsi-0QEMU_QEMU_HARDDISK_19353493-part3".try_into().unwrap());
